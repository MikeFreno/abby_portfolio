...
export default function CreateActingForm() {
  ...
  const createActingPage = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (titleRef.current && linkRef.current) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_DOMAIN}/api/database/acting/create`,
